...
def check_kalshi_markets(kalshi_file_path, max_temp_today, target_date_ticker_format):
    ...
    for market in markets:
        ...
        if not ticker.startswith("KXHIGHAUS"): continue
...
if __name__ == "__main__":
    ...
    if max_temp is None:
        ...
        if not pull_kalshi_data(): # Fetch latest markets into kalshi_markets.json
            ...
            if alert_list:
                subject = f"Kalshi Alert: Austin High Temp Market(s) Resolved for {TARGET_DATE_TICKER_STR}"
                body_intro = (f"The highest temperature in Austin for {TARGET_DATE_TEMP_API_STR} appears to have been reached "
