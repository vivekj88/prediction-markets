...
TEMP_API_URL = "https://api.mesowest.net/v2/stations/timeseries?STID=KNYC&showemptystations=1&units=temp|F,speed|kts,english&recent=4320&token=d8c6aee36a994f90857925cea26934be&complete=1&obtimezone=local"
...
if __name__ == "__main__":
    ...
    if max_temp is None:
        ...
        if not pull_kalshi_data(): # Fetch latest markets into kalshi_markets.json
            ...
            if alert_list:
                subject = f"Kalshi Alert: New York City High Temp Market(s) Resolved for {TARGET_DATE_TICKER_STR}"
                body_intro = (f"The highest temperature in New York City for {TARGET_DATE_TEMP_API_STR} appears to have been reached "
