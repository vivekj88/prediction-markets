...
TEMP_API_URL = "https://api.mesowest.net/v2/stations/timeseries?STID=KLAX&showemptystations=1&units=temp|F,speed|kts,english&recent=4320&token=d8c6aee36a994f90857925cea26934be&complete=1&obtimezone=local"
...
def check_kalshi_markets(kalshi_file_path, max_temp_today, target_date_ticker_format):
    ...
    for market in markets:
        ...
        if not ticker.startswith("KXHIGHLAX"): continue
        ticker_parts = ticker.split('-')
        if len(ticker_parts) < 3: continue
        ticker_date_str = ticker_parts[1]
        print(f"  - Ticker Date: {ticker_date_str}, Target Date: {target_date_ticker_format}")
...
if __name__ == "__main__":
    ...
    if max_temp is None:
        ...
        if not pull_kalshi_data(): # Fetch latest markets into kalshi_markets.json
            ...
            if alert_list:
                subject = f"Kalshi Alert: LA High Temp Market(s) Resolved for {TARGET_DATE_TICKER_STR}"
                body_intro = (f"The highest temperature in Los Angeles for {TARGET_DATE_TEMP_API_STR} appears to have been reached "
